...
export function getPortableProtocolExecutable(
  platform: string,
  executable?: string,
): string | undefined {
...
export class ProtocolManager {
  ...
  private registerProtocolHandlers(): void {
    ...
    // Portable builds extract into a temporary directory; register the persistent launcher.
    const portableExecutable = !isDev
      ? getPortableProtocolExecutable(process.platform, process.env.PORTABLE_EXECUTABLE_FILE)
      : undefined;
    const isCurrentlyRegistered = portableExecutable
      ? app.isDefaultProtocolClient(this.protocolScheme, portableExecutable)
      : app.isDefaultProtocolClient(this.protocolScheme);
    logger.debug(
    ...
    } else if (portableExecutable) {
      registrationResult = app.setAsDefaultProtocolClient(this.protocolScheme, portableExecutable);
    } else {
    ...
    // Verify registration
    const isRegisteredAfter = portableExecutable
      ? app.isDefaultProtocolClient(this.protocolScheme, portableExecutable)
      : app.isDefaultProtocolClient(this.protocolScheme);
    logger.debug(
