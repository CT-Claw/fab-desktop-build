...
const materializedOptionalNativePackagePatterns = [
  /^@lydell\/node-pty-/u,
  /^@napi-rs\/canvas-/u,
  /^node-screenshots-/u,
];
...
/**
 * Locate an installed package even when pnpm keeps an optional dependency only
 * in its virtual store instead of linking it at the workspace root. Cross-OS
 * Electron packaging needs this for target-specific native packages.
 */
function findInstalledModuleSource(moduleName) {
  const directPath = path.join(sourceNodeModules, moduleName);
  if (fs.existsSync(directPath)) return fs.realpathSync(directPath);

  const virtualStore = path.join(sourceNodeModules, '.pnpm');
  if (!fs.existsSync(virtualStore)) return undefined;

  const candidates = fs
    .readdirSync(virtualStore, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => path.join(virtualStore, entry.name, 'node_modules', moduleName))
  ...
  if (uniqueCandidates.length > 1) {
    throw new Error(
      `Multiple installed copies found for ${moduleName}; refusing to choose a native binary`,
    );
  }
...
export async function copyModulesToSource(modules, label, options = {}) {
  ...
  for (const dep of deps) {
    ...
    try {
      const stat = await fs.promises.lstat(modulePath).catch(() => undefined);
      if (stat && !stat.isSymbolicLink()) continue;

      if (
        !stat &&
        !materializedOptionalNativePackagePatterns.some((pattern) => pattern.test(dep))
      ) {
        console.info(`  ⏭️  ${dep} (skipped: package is not linked for this build host)`);
        continue;
      }

      const realPath = findInstalledModuleSource(dep);
      if (!realPath) {
        console.info(`  ⏭️  ${dep} (skipped: package is not installed for this build host)`);
        continue;
      }

      console.info(
        `  📎 ${dep} (${stat?.isSymbolicLink() ? 'resolving symlink' : 'materializing optional package'})`,
      );
      ...
      throw new Error(`Failed to prepare ${label} ${dep}: ${err.code || err.message}`, {
        cause: err,
      });
    }
  }
