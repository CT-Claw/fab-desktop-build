...
import { loadDesktopBuildProfile } from './desktop-build-profile.mjs';
...
export default defineConfig(async (env) => {
  ...
  const stagedProfile = await loadDesktopBuildProfile(process.env.DESKTOP_BUILD_PROFILE_PATH);
  const buildIdentity = stagedProfile?.profile;
  ...
  const config = {
    ...
    define: {
      ...processEnvDefine,
      'process.env.DESKTOP_APPLICATION_ID': JSON.stringify(
        buildIdentity?.applicationId ?? process.env.DESKTOP_APPLICATION_ID,
      ),
      'process.env.DESKTOP_APPLICATION_NAME': JSON.stringify(
        buildIdentity?.applicationName ?? process.env.DESKTOP_APPLICATION_NAME,
      ),
      'process.env.DESKTOP_EXTERNAL_NAVIGATION_HOSTS': JSON.stringify(externalNavigationHosts),
      'process.env.DESKTOP_OIDC_CLIENT_ID': JSON.stringify(
        buildIdentity?.oidcClientId ?? process.env.DESKTOP_OIDC_CLIENT_ID,
      ),
      'process.env.DESKTOP_PROTOCOL_SCHEME': JSON.stringify(
        buildIdentity?.protocolScheme ?? process.env.DESKTOP_PROTOCOL_SCHEME,
      ),
      'process.env.OFFICIAL_CLOUD_SERVER': JSON.stringify(
        buildIdentity?.officialCloudServer ?? process.env.OFFICIAL_CLOUD_SERVER,
      ),
