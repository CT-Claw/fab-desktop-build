...
export class AgentModel {
   ...
   */
  getAgentByMarketIdentifier = async (
    marketIdentifier: string,
    ownedPrivateOnly = false,
  ): Promise<string | null> => {
    const result = await this.db.query.agents.findFirst({
      columns: { id: true },
      ...
      where: and(
        ...
        this.ownership(),
        // Filter before ordering so newer shared entries cannot hide a private installation.
        ownedPrivateOnly ? eq(agents.userId, this.userId) : undefined,
        ownedPrivateOnly ? eq(agents.visibility, 'private') : undefined,
      ),
    });
