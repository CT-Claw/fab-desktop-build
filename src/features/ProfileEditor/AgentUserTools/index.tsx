...
import {
  getFabOfflinePluginEntries,
  isFabPluginPinned,
} from '@/features/LocalAssistantMarket/offlineProfile';
...
const AgentUserTools = memo<AgentToolProps>((props) => {
  ...
  const agentConfig = useAgentStore(agentSelectors.getAgentConfigById(effectiveAgentId), isEqual);

  useEffect(() => {
    if (effectiveAgentId && !isInit) fetchAgentConnectors(effectiveAgentId);
  }, [effectiveAgentId, isInit, fetchAgentConnectors]);

  // Migrate local assistants created before the Fab skill registry was added.
  // This only touches assistants from the local catalog and only adds missing
  // bundled plugin IDs; user-added tools and prompt edits remain untouched.
  useEffect(() => {
    const marker = 'local:fab.gdibao.com:';
    const identifier = agentConfig?.marketIdentifier?.startsWith(marker)
      ? agentConfig.marketIdentifier.slice(marker.length)
      : undefined;
    if (!identifier) return;

    const required = getFabOfflinePluginEntries(identifier).filter(
      (plugin) =>
        plugin.mode === 'pinned' && !isFabPluginPinned(agentConfig.plugins, plugin.identifier),
    );
    ...
    void updateAgentConfigById(effectiveAgentId, { plugins });
  }, [agentConfig, effectiveAgentId, updateAgentConfigById]);
