...
export const useCreateMenuItems = () => {
  ...
  const createGroupChatMenuItem = useCallback(
    (options?: CreateAgentOptions): MenuItem => ({
      ...
      label: <span data-testid="native-create-group">{t('newGroupChat')}</span>,
      sfSymbol: 'person.2',
      onClick: async (info) => {
        info.domEvent?.stopPropagation();
        if (!canCreate) return;

        const groupOptions = { ...options, visibility: options?.visibility ?? 'private' };
        if (openCreateModal) {
          openCreateModal('group', {
            ...(groupOptions.groupId ? { groupId: groupOptions.groupId } : {}),
            visibility: groupOptions.visibility,
          });
        } else {
          await createEmptyGroup(groupOptions);
        }
      },
    }),
