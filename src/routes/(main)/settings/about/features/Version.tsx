...
import { normalizeAboutPageConfig } from '@/const/aboutLinks';
...
const Version = memo<{ logoUrl?: string | null; mobile?: boolean }>(({ logoUrl, mobile }) => {
  ...
  const renderUpdateButton = () => {
    if (!isDesktop) {
      if (hasNewVersion) {
        return (
          <a
            href={CHITUO_PUBLIC_LINKS.downloads}
  ...
  const normalizedAboutPage = normalizeAboutPageConfig(aboutPage);
  const logoLinkUrl = normalizedAboutPage.logoLinkUrl;
  const changelogUrl = normalizedAboutPage.changelogUrl;
  const changelogLabel = normalizedAboutPage.changelogLabel || t('changelog');
