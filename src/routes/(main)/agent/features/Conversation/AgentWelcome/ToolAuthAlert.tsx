...
} from "@/store/tool/slices/composioStore";
...
const ComposioToolAuthItem = memo<ComposioToolAuthItemProps>(
  ({ tool, onAuthComplete }) => {
    ...
    const windowCheckIntervalRef = useRef<ReturnType<
      typeof setInterval
    > | null>(null);
    ...
    const createComposioConnection = useToolStore(
      (s) => s.createComposioConnection,
    );
    const refreshComposioConnectionStatus = useToolStore(
      (s) => s.refreshComposioConnectionStatus,
    );
    const removeComposioConnection = useToolStore(
      (s) => s.removeComposioConnection,
    );
    ...
    useEffect(() => {
      if (
        tool.server?.status === ComposioServerStatus.ACTIVE &&
        isWaitingAuth
      ) {
        cleanup();
        onAuthComplete();
      ...
      (identifier: string) => {
        ...
        pollIntervalRef.current = setInterval(async () => {
          try {
            ...
            console.info(
              "[Composio] Polling check (expected during auth):",
              error,
            );
          }
        }, POLL_INTERVAL_MS);
    ...
    const handleAuthorize = async () => {
      ...
      if (
        tool.server?.status === ComposioServerStatus.PENDING_AUTH &&
        tool.server.redirectUrl
      ) {
...
const ToolAuthAlert = memo(() => {
  ...
  const plugins = useAgentStore(
    agentByIdSelectors.getAgentPluginsById(agentId),
    isEqual,
  );
  const composioServers = useToolStore(
  ...
  const pendingAuthTools = useMemo<PendingAuthTool[]>(() => {
    ...
    for (const pluginId of plugins) {
      ...
      const composioType = COMPOSIO_APP_TYPES.find(
        (t) => t.identifier === pluginId,
      );
      if (composioType) {
      ...
      // Fab runs the sandbox through the server-side OnlyBoxes provider. It
      // must never fall back to the official market login flow.
    }

    return result;
  }, [plugins, composioServers, isComposioServersInit]);
