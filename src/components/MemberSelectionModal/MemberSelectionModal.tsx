...
const MemberSelectionModal = memo<MemberSelectionModalProps>(
  ({
    ...
    // The chat-context available hook is capped at 12; member selection needs the full native list.
    const {
      data: available,
      error: listError,
      mutate: refreshAvailable,
    } = useClientDataSWRWithSync<AvailableAgentItem[]>(
      open ? agentConfigKeys.search() : null,
    ...
    const handleConfirm = async () => {
      ...
      try {
        ...
        setIsAdding(true);
        await onConfirm(selectedAgents, normalizedHostModelConfig, !isHostRemoved);
        handleReset();
      } catch (error) {
    ...
    const isConfirmDisabled = selectedAgents.length === 0 || isAdding;
    ...
    return (
      <ImperativeModal
        ...
        footer={
          <Flexbox horizontal gap={8} justify="end">
            ...
            <Button
              data-testid="native-group-confirm"
      ...
      >
        {confirmError && <p role="alert">{t('sessionGroup.createGroupFailed')}</p>}
        {listError && (
          <p role="alert">
            <Button onClick={() => void refreshAvailable()}>{t('retry', { ns: 'common' })}</Button>
          </p>
        )}
        <Button
          data-testid="native-group-catalog"
          onClick={() => setShowCatalog((value) => !value)}
        >
          {t('localMarket.local', { ns: 'discover' })}
        </Button>
        {showCatalog && (
          <div style={{ maxHeight: '45vh', overflow: 'auto' }}>
            <LocalAssistantMarket
              ...
              onInstalled={async (agentId) => {
                ...
                if (!existingMembers.includes(agentId)) {
                  setSelectedAgents((ids) => (ids.includes(agentId) ? ids : [...ids, agentId]));
                }
