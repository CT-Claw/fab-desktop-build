...
const currentAgentAvatar = (s: AgentStoreState) =>
  resolveDefaultAgentAvatar(currentAgentData(s)?.avatar) ||
  getDefaultAvatarByAgentId(s, s.activeAgentId);
...
const currentAgentMeta = (s: AgentStoreState): MetaData => {
  ...
  return {
    avatar:
      resolveDefaultAgentAvatar(data?.avatar) || getDefaultAvatarByAgentId(s, s.activeAgentId),
...
const getAgentMetaById =
  ...
  (s: AgentStoreState): MetaData => {
    ...
    return {
      avatar: resolveDefaultAgentAvatar(data.avatar) || getDefaultAvatarByAgentId(s, agentId),
      backgroundColor: data.backgroundColor || DEFAULT_BACKGROUND_COLOR,
      description: data.description || undefined,
      marketIdentifier: data.marketIdentifier || undefined,
      tags: data.tags,
