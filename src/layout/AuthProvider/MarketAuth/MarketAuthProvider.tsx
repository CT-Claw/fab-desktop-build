...
const localMarketContext: MarketAuthContextType = {
...
// The local SPA never mounts token recovery, official login listeners or marketing modals.
// Official browsing is an explicit external link and retains its own authentication.
export const MarketAuthProvider = ({ localOnly, ...props }: MarketAuthProviderProps) =>
  localOnly ? (
    <MarketAuthContext value={localMarketContext}>{props.children}</MarketAuthContext>
  ) : (
    <ConnectedMarketAuthProvider {...props} />
  );
 ...
 */
const ConnectedMarketAuthProvider = ({ children, isDesktop }: MarketAuthProviderProps) => {
